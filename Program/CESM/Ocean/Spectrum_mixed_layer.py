#Program performs a red-noise test on the mixed layer depth
...
if moving_average > 1:
	...
	time_year, mixed_year	= MovingAverage(time_year, mixed_year, moving_average / 12)

time, data_series	= time_year, mixed_year
...
e_1	=  np.where(auto_corr < 1.0/np.e)[0][0] + 1
...
ax1.set_xlim([1/60.0,0.5])
ax1.set_xlabel('Frequency (year$^{-1}$)')
#ax1.set_ylim(0, 70)
ax2 = ax1.twiny()

new_tick_locations = np.array([1.0/50, 1.0/30, 1.0/20, 1.0/10, 1.0/5, 1.0/3, 1.0/2.0])
ax2.set_xlabel('Period (years)')
ax2.set_xscale('log')

ax2.set_xlim(ax1.get_xlim())
ax2.set_xticks(new_tick_locations)
ax2.set_xticklabels(YearConverter(new_tick_locations))
ax2.grid()
...
ax1.text(0.01, 0.95, 'a) Mixed layer',
