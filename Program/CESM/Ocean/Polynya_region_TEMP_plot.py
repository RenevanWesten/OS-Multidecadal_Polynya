#Program plots the Hovmoller diagram of temperature anomalies of the polynya region
...
if trend_type > 0:
	...
	for depth_i in range(len(depth)):
		temp_depth[:, depth_i]	= TrendRemover(time, temp_depth[:, depth_i], trend_type)
else:

	temp_depth	= temp_depth - np.mean(temp_depth, axis = 0)

if moving_average > 1:
	
	time_2, temp_2		= MovingAverage(time, temp_depth[:, 0], moving_average)
	temp_depth_2		= ma.masked_all((len(time_2), len(depth)))

	for depth_i in range(len(depth)):
		#Take moving average over each depth level
		time_2, temp_depth_2[:, depth_i]	= MovingAverage(time, temp_depth[:, depth_i], moving_average)
	
	#Save the smoothed files
	time		= time_2
	temp_depth	= temp_depth_2	
...
CS 	= ax1.contourf(x, y, temp_depth.transpose(), levels, extend = 'both', cmap = 'RdBu_r')

fig.subplots_adjust(right=0.8)
cbar 	= fig.add_axes([0.82, 0.10, 0.025, 0.8])
fig.colorbar(CS, cax = cbar, ticks = np.arange(-1, 1.01, 0.5))
cbar.set_ylabel('Temperature anomaly ($^{\circ}$C)')
...
CS 		= ax2.contourf(x, y, temp_depth.transpose(), levels, extend = 'both', cmap = 'RdBu_r')
...
ax1.set_title('c) Temperature and mixed layer')
