#Program plots the Hovmoller diagram of salinity anomalies of the polynya region
...
if trend_type > 0:
	...
	for depth_i in range(len(depth)):
		salt_depth[:, depth_i]	= TrendRemover(time, salt_depth[:, depth_i], trend_type)
else:

	salt_depth	= salt_depth - np.mean(salt_depth, axis = 0)

if moving_average > 1:
	
	time_2, salt_2		= MovingAverage(time, salt_depth[:, 0], moving_average)
	salt_depth_2		= ma.masked_all((len(time_2), len(depth)))

	for depth_i in range(len(depth)):
		#Take moving average over each depth level
		time_2, salt_depth_2[:, depth_i]	= MovingAverage(time, salt_depth[:, depth_i], moving_average)
	
	#Save the smoothed files
	time		= time_2
	salt_depth	= salt_depth_2	
...
CS 	= ax1.contourf(x, y, salt_depth.transpose(), levels, extend = 'both', cmap = 'RdBu_r')

fig.subplots_adjust(right=0.8)
cbar 	= fig.add_axes([0.82, 0.10, 0.025, 0.8])
fig.colorbar(CS, cax = cbar, ticks = np.arange(-0.1, 0.11, 0.05))
cbar.set_ylabel('Salinity anomaly (Psu)')
...
CS 		= ax2.contourf(x, y, salt_depth.transpose(), levels, extend = 'both', cmap = 'RdBu_r')
...
ax1.set_title('d) Salinity and mixed layer')
